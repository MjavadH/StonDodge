...
func _physics_process(delta: float) -> void:
	match current_state:
		...
		State.DASHING, State.RECOIL, State.BACK:
			var current_speed = speed * (1.5 if current_state == State.RECOIL else 1.0)
			...
			if global_position.distance_to(target_position) < stop_distance:
				if current_state == State.BACK:
					...
					var random_wait = randi() % 3 + 3 # randi_range(3, 5)
	...
	if collision and collision.get_collider() == character_node and current_state == State.DASHING:
		var recoil_direction = collision.get_normal().rotated(randf_range(-0.5, 0.5))
...
func _on_animation_player_animation_finished(anim_name: StringName) -> void:
	match anim_name:
		"ShowSelf":
			change_state(State.IDLE)
			var random_wait = randi() % 2 + 2 # Modern equivalent of randi_range(2, 3)
			action_timer.start(random_wait)
		"preFallowAnim":
